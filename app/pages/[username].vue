...
const { data: publicUser } = useQuery({
  key: ['publicUser'],
  query: () => useRequestFetch()(`/api/user/${route.params.username}`) as Promise<User>,
});

const { data: habits } = useQuery({
  key: ['habits'],
  query: () => useRequestFetch()(`/api/user/${route.params.username}/habits`) as Promise<Habit[]>,
});
